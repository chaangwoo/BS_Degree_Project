...
if __name__ == '__main__':
    dirs = next(os.walk('.'))[1]
    # print(dirs)
    write_to_csv(['workload', 'numCycles', 'tlb_hits', 'read_hits', 'write_hits', 'tlb_misses', 'read_misses', 'write_misses', 'mshr_hits', 'mshr_stall_cycles', 'num_jobs', 'total_cycles', 'num_tasks', 'read_action_cycles', 'compute_action_cycles', 'write_action_cycles'])
    for sub_dir in dirs:
        print(sub_dir)
        if (sub_dir == FILES[8].split("/")[0]):
            print("  skip " + sub_dir + "...")
            continue
        ...
        for i in range(1, acc_count):
            comps = [0, i, tlb_hits[i], read_hits[i], write_hits[i], tlb_misses[i], read_misses[i], write_misses[i], mshr_hits[i], mshr_stall_cycles[i], num_jobs[i], total_cycles[i], num_tasks[i], read_action_cycles[i], compute_action_cycles[i], write_action_cycles[i]]
            write_to_csv(comps)
