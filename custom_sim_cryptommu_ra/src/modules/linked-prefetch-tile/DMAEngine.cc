...
TransferSetDesc::NextTransfer()
{
  ...
  /* Generate single transfer info (base&size) which fits in a single cacheline */
...
DMAEngine::DMAEngine()
{
  ...
  // changed
  // dmaLifeTime = 123123;
  dmaTimeStamp = GetSystemTime();
  stallTimeStamp = GetSystemTime();
  busyCycles = 0;
  idleCycles = 0;
  stallCycles = 0;
  isStalled = false;
...
DMAEngine::~DMAEngine()
{
  ...
  // changed
  // dmaLifeTime = 456456;
  // idleCycles += GetSystemTime() - dmaTimeStamp;
  //
}
...
DMAEngine::TryTransfers()
{
  ...
  // changed
  if (isStalled) {
    stallCycles += GetSystemTime() - stallTimeStamp;
    isStalled = false;
  }
  //
...
//  ML_LOG("DMAEngine", "Try transfer TD" << td << " issued 0x" << std::hex << td->getVaddr() << std::dec
...
DMAEngine::OnMemoryResponse(uint64_t addr, uint64_t calledTime)
{
  ...
  // changed
  if (inflight == 0) {
    busyCycles += GetSystemTime() - dmaTimeStamp;
    dmaTimeStamp = GetSystemTime();
  }
  //
