...
class DMAEngine
{
  ...
  // changed
  uint64_t dmaLifeTime;
  uint64_t busyCycles;
  uint64_t idleCycles;  
  uint64_t stallCycles; // stall due to the DMA issue width
  uint64_t dmaTimeStamp;
  uint64_t stallTimeStamp;
  bool isStalled;
  //
...
public:
  ...
  // changed
  uint64_t getDmaLifeTime() {
    return dmaLifeTime;
  }
  uint64_t getBusyCycles() {
    return busyCycles;
  }
