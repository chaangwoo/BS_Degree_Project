...
//    ML_LOG(GetDeviceName(), "TD_CMD_BEGIN_PROGRAM Virtual Address 0x"<< std::hex << logicalAddr
//        << " Physical Address 0x" << physicalAddr << std::dec << " src=" << src << " process=" << process);
...
//    ML_LOG(GetDeviceName(),
//           "TLB miss received from device " << src 
//           << " for userthread " << process << " LA 0x" << std::hex << logicalAddr << " PA 0x" << phyAddr << std::dec);
...
//    if (logicalAddr%PAGE_SIZE!=0 || physicalAddr%PAGE_SIZE!=0)
//    {
//      ML_LOG(GetDeviceName(), "LCACC_CMD_TLB_SERVICE Virtual Address 0x"<< std::hex << logicalAddr
//	      << " Physical Address 0x" << physicalAddr << std::dec 
...
//                  << std::hex << logicalPage << " -> 0x" << physicalPage << std::dec << " for thread" << myThreadID << " device " << requester);
          }
        }
      ...
      else
      {
        ...
        for (size_t i = 0; i < cfuTlbMisses[process][logicalPage].size(); i++) {
          if (cfuTlbMisses[process][logicalPage][i] != 0) {
            ...
            netPort->SendMessage(requester, outMsg, sizeof(outMsg)); /*Page walk miss in BCC, search latency and memeory access*/
//	    ML_LOG(GetDeviceName(), "LCAcc TLB miss serviced 0x"
//                  << std::hex << logicalPage << " -> 0x" << physicalPage << " MAC=" << MAC << std::dec << " for thread" << myThreadID << " device " << requester);
          }
        }
        cfuTlbMisses[process].erase(logicalPage);
...
TD::ReadProgramDone()
{
  ...
  ML_LOG(GetDeviceName(), "END PRG RD for thread " << trd.process << std::hex 
		  << " logicalAddr 0x" << trd.logicalAddr << " physicalAddr 0x" << trd.physicalAddr << std::dec);
  assert(trd.loadedPacket == NULL);
...
//  ML_LOG(GetDeviceName(), "Start PRG RD for thread " << trd.process << std::hex 
//      << " logicalAddr 0x" << trd.logicalAddr << " physicalAddr 0x" << trd.physicalAddr << std::dec);
...
TD::HandleTLBMiss(uint64_t addr)
{
  ...
  BitConverter bc_paddr_dummy;
  bc_paddr_dummy.u64[0] = 0;
  msg[4] = bc_paddr_dummy.u32[0];
  msg[5] = bc_paddr_dummy.u32[1];

  BitConverter bc_MAC_dummy;
  bc_MAC_dummy.u64[0] = 0;
  msg[6] = bc_MAC_dummy.u32[0];
  msg[7] = bc_MAC_dummy.u32[1];

  BitConverter bc_deviceid;
  bc_deviceid.u64[0] = netPort->GetNodeID();
  msg[8] = bc_deviceid.u32[0];
  msg[9] = bc_deviceid.u32[1];

//  ML_LOG(GetDeviceName(), "TLB miss on TD DMA vaddr 0x"
//         << std::hex << bc.u64[0] << " paddr 0x" << bc_paddr_dummy.u64[0] << std::dec << " MAC" << bc_MAC_dummy.u64[0] << " device" << bc_deviceid.u64[0]);
