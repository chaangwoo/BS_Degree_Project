...
//    ML_LOG(GetDeviceName(), "Got TLB miss for page 0x" << std::hex << vAddr
//		  << " physical page 0x" << pAddr << std::dec << " src " << nih->deviceID 
//		  << " device " << device_id << " thread " << thread << " MAC " << MAC);
    /*
...
void
NetworkInterrupts::PeekFaultState()
{
  assert(pendingRetire.empty()==false);

  /* Probe request */
  WholeTranslationState* state = pendingRetire.front();
  ...
  /* Peek status in PTE directly (referenced from arch/x86/vtophys) */
  unsigned logBytes;
  Addr tmp_addr = vaddr;
  Fault fault = walker->startFunctional(tc, tmp_addr, logBytes, BaseTLB::Read);

  if (fault != NoFault) {
    /* Still need to wait... */
    EnqueueEvent(PeekFaultStateCB::Create(this), interval);
    return;
  }

  Addr masked_addr = vaddr & mask(logBytes);
  Addr paddr = tmp_addr | masked_addr;
  req->setPaddr(paddr);

  /* Respond as usual */
  state->setNoFault();
  finishTranslation(state);
  pendingRetire.pop_front();

  if (pendingRetire.empty()==false) 
    EnqueueEvent(PeekFaultStateCB::Create(this), interval);
}
...
void NetworkInterrupts::finishTranslation(WholeTranslationState *state)
{
  ...
  {
//    panic("Translation encountered fault (%s) for address 0x%x, hasPaddr=%d\n",
//          state->getFault()->name(), state->mainReq->getVaddr(), state->mainReq->hasPaddr());
    printf("Translation encountered fault (%s) for address 0x%x, hasPaddr=%d\n",
          state->getFault()->name(), state->mainReq->getVaddr(), state->mainReq->hasPaddr());

    /* 
     * Corner case: PTE might be touched before host allocation due to async. operations 
     * We assume PTE is ensured to be always allocated in this work for simplicity by spinning
     */
    pendingRetire.push_back(state);
    if (pendingRetire.size()==1) {
      EnqueueEvent(PeekFaultStateCB::Create(this), interval);
    }

    /* Prevent perf dependency due to unwanted faults */
  ...
  ML_LOG(GetDeviceName(), "Return logical page 0x" << std::hex << logicalPage
		  << " physical page 0x" << physicalPage << std::dec 
		  << " src " << nih->deviceID << " device " << device_id << " thread " << thread << " MAC " << MAC);
  if (pendingTranslations.size())
  {
