  ...
  if (a->readSet.size() > 0) {
    ...
    for (size_t i = 0; i < a->readSet.size(); i++) {
      ...
      if (pendingReadActions.empty()) {
        readTimeStamp = SimicsInterface::GetSystemTime();
      }
      pendingReadActions.insert(a);

      if (pendingRWActions.empty()) {
        RWTimeStamp = SimicsInterface::GetSystemTime();
      }
      pendingRWActions.insert(a);
      //
...
LCAccDevice::FinishedActionRead(Action* a, size_t index)
{
  ...
  if (a->readsInFlight == 0) {
    ...
    StartTask();
    // changed
    readCycles += SimicsInterface::GetSystemTime() - actionTimeStamp[a];

    pendingReadActions.erase(a);
    if (pendingReadActions.empty()) {
      readActionTime += SimicsInterface::GetSystemTime() - readTimeStamp;
    }

    pendingRWActions.erase(a);
    if (pendingRWActions.empty()) {
      RWActionTime += SimicsInterface::GetSystemTime() - RWTimeStamp;
    }
    //
    EmitActionCompute(a);
  }
...
LCAccDevice::EmitActionCompute(Action* a)
{
  ...
  // changed
    ...
    if (pendingComputeActions.empty()) {
      computeTimeStamp = SimicsInterface::GetSystemTime();
    }
    pendingComputeActions.insert(a);
  //
...
LCAccDevice::TryRetireComputeElement(ComputeElement* ce)
{
  ...
  if (ce->writesRemaining == 0 && ce->randomAccessesRemaining == 0) {
    ...
    if (ce->host->pendingComputeElements == 0) {
      ...
      // changed
      computeCycles += SimicsInterface::GetSystemTime() - actionTimeStamp[ce->host->host];

      pendingComputeActions.erase(ce->host->host);
      if (pendingComputeActions.empty()) {
        computeActionTime += SimicsInterface::GetSystemTime() - computeTimeStamp;
      }
      //
      SimicsInterface::RegisterCallback(
...
LCAccDevice::EmitActionWrites(Action* a)
{
  ...
  if (pendingWriteActions.empty()) {
    writeTimeStamp = SimicsInterface::GetSystemTime();
  }
  pendingWriteActions.insert(a);
  ...
  if (a->writeSet.size() > 0) {
    ...
    // changed
    writeCycles += SimicsInterface::GetSystemTime() - actionTimeStamp[a];
    actionTimeStamp.erase(a);
    
    pendingWriteActions.erase(a);
    if (pendingWriteActions.empty()) {
      writeActionTime += SimicsInterface::GetSystemTime() - writeTimeStamp;
    }
...
void LCAccDevice::HandleTLBMiss(uint64_t addr, uint64_t MAC_code, uint64_t paddr)
{
  ...
  //std::cout << "Node Id Handle TLB miss " << netPort->GetNodeID() << std::endl;
    //assert(msg[2] == addr);
  ML_LOG(GetDeviceName(), "meta vaddr: paddr : MAC_code 0x" 
		  << std::hex << addr << " 0x" << paddr << " " << MAC_code << " device " <<netPort->GetNodeID());
...
LCAccDevice::Initialize(uint32_t threadID)
{
  ...
  numHandleTLBMissCall_0 = 0;
  numHandleTLBMissCall_1 = 0;
  numHandleTLBMissCall_2 = 0;
  //
