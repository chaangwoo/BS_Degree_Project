...
if __name__ == '__main__':
    ...
    for sub_dir in dirs:
        ...
        for i in range(1, acc_count):
            comps = [0, i, tlb_hits[i], read_hits[i], write_hits[i], tlb_misses[i], read_misses[i], write_misses[i], mshr_hits[i], 0, num_jobs[i], total_cycles[i], num_tasks[i], read_action_cycles[i], compute_action_cycles[i], write_action_cycles[i]]
            write_to_csv(comps)
